...
# table1.forces[2] = (x, t) -> -table2.mass * g
sys = MBSystem2D()
add!(sys, ground)
...
# motor1 = PositionMotor2D(hinge_gr2cr1, π/3)
# motor2 = PositionMotor2D(hinge_gr2cr2, π/3)
# motor3 = PositionMotor2D(hinge_gr2cr3, -π/3)
# motor4 = PositionMotor2D(hinge_gr2cr4, -π/3)
...
for body in sys.bodies
    p1, p2 = get_boundary_points(sys, body, initial_state)
    lines!(ax, [p1, p2])
end

for conn in sys.connectors
    if conn isa HingeJoint
        point = Flexia.get_hinge_point(sys, conn, initial_state)
        scatter!(ax, point, color=:red, markersize=5)
    end
end
...
time_span = 0:0.001:0.05

# Solve
sol = simulate(sys, initial_state, time_span)

# Animate
animate(sys, sol, time_span, "crankshaft2.mp4"; framerate=1, limits=(-2, 5, -2, 2))
