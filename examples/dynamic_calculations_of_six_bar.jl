...
set_position_on_first_body!(ground_rail_hinge1, SA[0., actuator_offset_Y])
set_position_on_second_body!(ground_rail_hinge1, SA[0., 0.])

tcp_ground_rail_1 = TorsionalSpring(bd1, slider_rail_1, K₁, deg2rad(0), 0.1, 0.03)
...
set_position_on_second_body!(hor_bd2_hinge, SA[-(bd2.length/2-actuator_offset_Y), 0])

tcp_hor_bd2 = TorsionalSpring(slider_1, bd2, K₁, deg2rad(0), 0.1, 0.03)

# right side with actuator

ground_rail_hinge2 = HingeJoint(bd7, slider_rail_2)
set_position_on_first_body!(ground_rail_hinge2, SA[0., actuator_offset_Y])
set_position_on_second_body!(ground_rail_hinge2, SA[0., 0.])

tcp_ground_rail_2 = TorsionalSpring(bd7, slider_rail_2, K₁, deg2rad(0), 0.1, 0.03)

hor_bd6_hinge = HingeJoint(bd6, slider_2)
set_position_on_first_body!(hor_bd6_hinge, SA[-(actuator_offset_Y-bd6.length/2), 0])
set_position_on_second_body!(hor_bd6_hinge, SA[-(slider_2.length/2), 0])

tcp_hor_bd6 = TorsionalSpring(bd6, slider_2, K₁, deg2rad(0), 0.1, 0.03)
...
initial[slider_rail_1_Y] = actuator_offset_Y
...
initial[slider_2_Y] = actuator_offset_Y
...
initial[bd4_x_ind] = bd1.length/2 + bd4.length/2 + bd3.length*cos(deg2rad(45))
initial[bd4_y_ind] = bd2.length + bd3.length*sin(deg2rad(45))
initial[bd4_t_ind] = deg2rad(0)

initial[bd5_x_ind] = bd1.length/2 + bd2_bd6_offset_x - 0.15*cos(deg2rad(45))/2
