...
function update_stiffnesses!( ω_exp, initial_stiffness, stiffness_groups, target_deg; α=0.3, k_min=1e-8, tol=1e-6, max_iter=50)
    ...
    for m in 1:max_iter
        ...
        T = compute_sensitivity_matrix(sys_new, q_eq, stiffness_groups)
        T_inv = pinv(T[:, :])
...
# target_angles = [90, 100]
# K_hsp = 5000.
# offset, _ = find_offset(K_hsp, target_deg = 100 )
# hinge_x_offset = offset
# sys_test, sol_test, initial_test, angle = build_6bar_system(hinge_x_offset, K_hsp)
# # Задаем время (если оно не задано глобально)
# omega = 0.5
# t_end = 10 * π / omega
# time_span = LinRange(0, t_end, 501)
# animate(sys_test, sol_test, time_span, "Flexia/out/new_6bar_test.mp4"; framerate = 30, limits = (-0.1, 0.6, -0.1, 0.5))
...
k = [0.01, 50.0]
ω_exp_80 = [8, 33]
ω_exp_90 = [7, 29]# эксп первая и вторая частоты
ω_exp_100 = [6, 36]
...
k_result_80, status_80 = update_stiffnesses!( ω_exp_80, k, gr, 80; α=0.3, max_iter=4500, tol=1e-3)
k_result_90, status_90 = update_stiffnesses!( ω_exp_90, k, gr, 90; α=0.3, max_iter=4500, tol=1e-3)
k_result_100, status_100 = update_stiffnesses!( ω_exp_100, k, gr, 100; α=0.3, max_iter=4500, tol=1e-3)


k_80_test = [0.1683, 8000.0]
offset, _ = find_offset(k_80_test[1], k_80_test[2], 80)
sys_test, sol_test, _, angle = build_6bar_system(0.0, k_80_test[1], k_80_test[2])
Flexia.find_natural_freqs(sys_test, sol_test[:, end])